...
# 3. Deploy vLLM engine (pods must carry the llm-d-role=rollout-engine label)
...
The rollout controller discovers vLLM pods via `--engine-selector=llm-d-role=rollout-engine` and watches for pod readiness changes automatically. The trainer loads Llama-3.2-1B, perturbs weights (simulating gradient steps), and broadcasts them to vLLM via NCCL. The controller orchestrates the sleep/wake/sync lifecycle. See `deploy/cks/` for the full manifests.
