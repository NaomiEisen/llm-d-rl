...
class LlmdVerlCheckpointEngineManager:
    ...
    def __init__(self, config, trainer, replicas=None):
        ...
        import llmd_verl.checkpoint_engine  # noqa: F401
    ...
    def update_weights(self, global_steps: int | None = None) -> int:
    ...
    def _init_nccl_group(self) -> None:
        ...
        while time.time() < _deadline:
            try:
                ...
                _s.settimeout(1)
                _s.connect((master_meta["master_address"], master_meta["master_port"]))
                _s.close()
                print(f"[LLMD DEBUG] Trainer TCPStore is listening on {master_meta['master_address']}:{master_meta['master_port']}", flush=True)
                break
            except OSError:
                time.sleep(0.5)
        else:
            raise RuntimeError(
                f"Trainer rank 0 never started listening on "
                f"{master_meta['master_address']}:{master_meta['master_port']} "
                f"within 30s"
            )
        ...
        #    This drains the FSDP weight generator WITHOUT doing the NCCL broadcast,
        #    so vLLM does not need to participate.
        ray.get(
            self.trainer.execute_checkpoint_engine(
        ...
        ray.get(self.trainer.update_weights(global_steps=None))
        ...
        print(f"[LLMD] Collected metadata: {len(self._param_metadata['param_names'])} params", flush=True)
